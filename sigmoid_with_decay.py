...
def save_his():
    with open(filename, 'wb') as outfile:
        pickle.dump(wd_history, outfile, protocol=pickle.HIGHEST_PROTOCOL)
...
if HEAD.startswith('FC'):
    ...
    filename = 'history/INDEX_FC_{}_wd_{}_addition.pkl'.format(HEAD, NUM_DISTR)
history = {}
...
wd_history = {}
for wd in np.linspace(.1, .5, num=5, endpoint=True):
    model.train_model(trainloader, verbose=1, weight_decay=wd)
...
his = load_his(filename)
fig, (ax1, ax2) = plt.subplots(1, 2, sharex=True)

for wd in np.linspace(.1, .5, num=5, endpoint=True):
    res = []
    for eps in eps_choices:
        res.append(his[wd][eps][0])
    ax1.plot(res, label=str(wd)[:4], linestyle='--')
ticks = []
...
for wd in np.linspace(.1, .5, num=5, endpoint=True):
    res = []
    for eps in eps_choices:
        res.append(his[wd][eps][1])
    ax2.plot(res, label=str(wd)[:4], linestyle='--')
ticks = []
for i in range(len(eps_choices)):
    if i % 5 ==0:
        ticks.append(str(eps_choices[i])[:4])
...
ax2.legend()
plt.tight_layout()
plt.savefig('./res_plots/Sigmoid_weight_decay.png')